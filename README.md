[The Linux Kernel](https://www.infradead.org/~mchehab/kernel_docs/index.html)

+   [](https://www.infradead.org/~mchehab/kernel_docs/index.html)»
+   [Translations](https://www.infradead.org/~mchehab/kernel_docs/translations/index.html) »
+   [中文翻译](https://www.infradead.org/~mchehab/kernel_docs/translations/zh_CN/index.html) »
+   [与Linux 内核社区一起工作](https://www.infradead.org/~mchehab/kernel_docs/translations/zh_CN/process/index.html) »
+   Linux 内核代码风格
+   [View page source](https://www.infradead.org/~mchehab/kernel_docs/_sources/translations/zh_CN/process/coding-style.rst.txt)
...
[](https://www.infradead.org/~mchehab/kernel_docs/process/coding-style.html#codingstyle)[Linux kernel coding style](https://www.infradead.org/~mchehab/kernel_docs/process/coding-style.html)
...
## Linux 内核代码风格[¶](#linux "Permalink to this headline")
...
## 1) 缩进[¶](#id1 "Permalink to this headline")
...
## 2) 把长的行和字符串打散[¶](#id2 "Permalink to this headline")
...
## 3) 大括号和空格的放置[¶](#id3 "Permalink to this headline")
...
### 3.1) 空格[¶](#id4 "Permalink to this headline")

Linux 内核的空格使用方式 (主要) 取决于它是用于函数还是关键字。(大多数) 关键字 后要加一个空格。值得注意的例外是 sizeof, typeof, alignof 和 \_\_attribute\_\_，这 些关键字某些程度上看起来更像函数 (它们在 Linux 里也常常伴随小括号而使用，尽管 在 C 里这样的小括号不是必需的，就像 `struct fileinfo info;` 声明过后的 `sizeof info`)。

所以在这些关键字之后放一个空格:

if, switch, case, for, do, while

但是不要在 sizeof, typeof, alignof 或者 \_\_attribute\_\_ 这些关键字之后放空格。 例如，
...
## 4) 命名[¶](#id5 "Permalink to this headline")
...
## 5) Typedef[¶](#typedef "Permalink to this headline")
...
> 5.  可以在用户空间安全使用的类型。
>     
>     在某些用户空间可见的结构体里，我们不能要求 C99 类型而且不能用上面提到的 `u32` 类型。因此，我们在与用户空间共享的所有结构体中使用 \_\_u32 和类似 的类型。
>     
...
## 6) 函数[¶](#id6 "Permalink to this headline")
...
## 7) 集中的函数退出途径[¶](#id7 "Permalink to this headline")
...
选择一个能够说明 goto 行为或它为何存在的标签名。如果 goto 要释放 `buffer`, 一个不错的名字可以是 `out_free_buffer:` 。别去使用像 `err1:` 和 `err2:` 这样的GW\_BASIC 名称，因为一旦你添加或删除了 (函数的) 退出路径，你就必须对它们 重新编号，这样会难以去检验正确性。
...
int fun(int a)
{
        ...
        buffer \= kmalloc(SIZE, GFP\_KERNEL);
        if (!buffer)
                return \-ENOMEM;
...
## 8) 注释[¶](#id8 "Permalink to this headline")
...
## 9) 你已经把事情弄糟了[¶](#id9 "Permalink to this headline")
...
## 10) Kconfig 配置文件[¶](#kconfig "Permalink to this headline")
...
要查看配置文件的完整文档，请看 [Kconfig Language](https://www.infradead.org/~mchehab/kernel_docs/kbuild/kconfig-language.html)。

## 11) 数据结构[¶](#id10 "Permalink to this headline")
...
这种 `多级引用计数` 的例子可以在内存管理 (`struct mm_struct`: mm\_users 和 mm\_count)，和文件系统 (`struct super_block`: s\_count 和 s\_active) 中找到。

记住：如果另一个执行线索可以找到你的数据结构，但这个数据结构没有引用计数器， 这里几乎肯定是一个 bug。

## 12) 宏，枚举和RTL[¶](#rtl "Permalink to this headline")
...
3.  作为左值的带参数的宏： FOO(x) = y；如果有人把 FOO 变成一个内联函数的话，这 种用法就会出错了。
    
4.  忘记了优先级：使用表达式定义常量的宏必须将表达式置于一对小括号之内。带参数 的宏也要注意此类问题。
...
## 13) 打印内核消息[¶](#id11 "Permalink to this headline")
...
<linux/device.h> 里有一些驱动模型诊断宏，你应该使用它们，以确保信息对应于正确 的设备和驱动，并且被标记了正确的消息级别。这些宏有：dev\_err(), dev\_warn(), dev\_info() 等等。对于那些不和某个特定设备相关连的信息，<linux/printk.h> 定义 了 [`pr_notice()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/printk-basics.html#c.pr_notice "pr_notice"), [`pr_info()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/printk-basics.html#c.pr_info "pr_info"), [`pr_warn()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/printk-basics.html#c.pr_warn "pr_warn"), [`pr_err()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/printk-basics.html#c.pr_err "pr_err") 和其他。

写出好的调试信息可以是一个很大的挑战；一旦你写出后，这些信息在远程除错时能提 供极大的帮助。然而打印调试信息的处理方式同打印非调试信息不同。其他 pr\_XXX() 函数能无条件地打印，[`pr_debug()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/printk-basics.html#c.pr_debug "pr_debug") 却不；默认情况下它不会被编译，除非定义了 DEBUG 或设定了 CONFIG\_DYNAMIC\_DEBUG。实际这同样是为了 dev\_dbg()，一个相关约定是在一 个已经开启了 DEBUG 时，使用 VERBOSE\_DEBUG 来添加 dev\_vdbg()。

许多子系统拥有 Kconfig 调试选项来开启 -DDEBUG 在对应的 Makefile 里面；在其他 情况下，特殊文件使用 #define DEBUG。当一条调试信息需要被无条件打印时，例如， 如果已经包含一个调试相关的 #ifdef 条件，printk(KERN\_DEBUG …) 就可被使用。

## 14) 分配内存[¶](#id12 "Permalink to this headline")

内核提供了下面的一般用途的内存分配函数： [`kmalloc()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/mm-api.html#c.kmalloc "kmalloc"), [`kzalloc()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/mm-api.html#c.kzalloc "kzalloc"), [`kmalloc_array()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/mm-api.html#c.kmalloc_array "kmalloc_array"), [`kcalloc()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/mm-api.html#c.kcalloc "kcalloc"), [`vmalloc()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/mm-api.html#c.vmalloc "vmalloc") 和 [`vzalloc()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/mm-api.html#c.vzalloc "vzalloc")。 请参考 API 文档以获取有关它们的详细信息。
...
## 15) 内联弊病[¶](#id13 "Permalink to this headline")
...
一个基本的原则是如果一个函数有 3 行以上，就不要把它变成内联函数。这个原则的一 个例外是，如果你知道某个参数是一个编译时常量，而且因为这个常量你确定编译器在 编译时能优化掉你的函数的大部分代码，那仍然可以给它加上 inline 关键字。 [`kmalloc()`](https://www.infradead.org/~mchehab/kernel_docs/core-api/mm-api.html#c.kmalloc "kmalloc") 内联函数就是一个很好的例子。
...
## 16) 函数返回值及命名[¶](#id14 "Permalink to this headline")
...
比如， `add work` 是一个命令，所以 add\_work() 在成功时返回 0，在失败时返回 -EBUSY。类似的，因为 `PCI device present` 是一个判断，所以 [`pci_dev_present()`](https://www.infradead.org/~mchehab/kernel_docs/driver-api/pci/pci.html#c.pci_dev_present "pci_dev_present") 在成功找到一个匹配的设备时应该返回 1，如果找不到时应该返回 0。
...
## 17) 不要重新发明内核宏[¶](#id15 "Permalink to this headline")
...
## 19) 内联汇编[¶](#id17 "Permalink to this headline")
...
asm ("magic %reg1, #42\\n\\t"
     "more\_magic %reg2, %reg3"
     : /\* outputs \*/ : /\* inputs \*/ : /\* clobbers \*/);

## 20) 条件编译[¶](#id18 "Permalink to this headline")
...
如果你有一个在特定配置中，可能变成未使用的函数或变量，编译器会警告它定义了但 未使用，把它标记为 \_\_maybe\_unused 而不是将它包含在一个预处理条件中。(然而，如 果一个函数或变量总是未使用，就直接删除它。)

在代码中，尽可能地使用 IS\_ENABLED 宏来转化某个 Kconfig 标记为 C 的布尔 表达式，并在一般的 C 条件中使用它：

if (IS\_ENABLED(CONFIG\_SOMETHING)) {
        ...
}
...
#ifdef CONFIG\_SOMETHING
...
#endif /\* CONFIG\_SOMETHING \*/
